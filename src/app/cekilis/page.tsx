...
export default function CekilisPage() {
  ...
  return (
    <div className="p-5 pt-8">
      ...
      <div className="relative mb-6">
        ...
        <div className="relative flex items-start justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight">Çekiliş</h1>
            <p className="text-white/40 text-sm mt-1">Her satıra bir isim — kavanozu karıştır</p>
          </div>
      ...
      >
        {drawing ? "Çekiliyor…" : "🎯 Çekiliş Yap"}
      </button>

      {names.length < 2 && text.length > 0 && (
        <p className="text-center text-xs text-white/25 mt-2">En az 2 isim gerekli</p>
      )}
      ...
      {history.length > 0 && (
        <div className="mt-6">
          <div className="text-[10px] text-white/25 uppercase tracking-[0.15em] mb-3">Geçmiş</div>
          <div className="space-y-1.5">
            {history.map((name, i) => (
              <div
                key={i}
                className="flex items-center gap-3 rounded-xl px-4 py-2.5 bg-white/[0.03] border border-white/[0.04]"
              >
                <span className="text-white/20 text-xs w-4 tabular-nums">{i + 1}</span>
                <span className="text-sm text-white/65">{name}</span>
                {i === 0 && (
                  <span className="ml-auto text-[10px] text-amber-400 font-semibold uppercase tracking-wide">
                    son
                  </span>
                )}
              </div>
