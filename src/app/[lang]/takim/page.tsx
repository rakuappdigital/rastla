...
export default function TakimPage() {
  ...
  const handleReset = () => { clearText(); clearTeamCount(); setTeams([]); setShowReset(false); };
  ...
  return (
    <div className="p-5 pt-8">
      ...
      <div className="rounded-2xl p-4 mt-4 mb-5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="text-[10px] font-semibold text-white/40 uppercase tracking-[0.15em] mb-3">{d.teams.teamCountLabel}</div>
        <div className="flex gap-2 flex-wrap">
          {[2, 3, 4, 5, 6].map((n) => (
            <button key={n} onClick={() => { setTeamCount(n); setTeams([]); }}
      ...
      <button onClick={build} disabled={names.length < teamCount}
        className="w-full py-4 rounded-2xl font-bold text-base tracking-wide transition-all active:scale-[0.97] disabled:opacity-30 disabled:cursor-not-allowed bg-gradient-to-b from-green-400 to-green-600 text-white shadow-[0_4px_0_rgba(0,0,0,0.3),0_0_30px_rgba(34,197,94,0.2)] active:shadow-[0_1px_0_rgba(0,0,0,0.3)] active:translate-y-px"
      >
        {d.teams.build}
      </button>
      ...
      {names.length > 0 && names.length < teamCount && (
        <p className="text-center text-xs text-white/25 mt-2">
          {d.teams.minPrefix} {teamCount} {d.teams.minSuffix} ({names.length}/{teamCount})
        </p>
      )}
      ...
      {teams.length > 0 && (
        <div key={animKey} className="mt-6 space-y-3">
          {teams.map((team, i) => {
            ...
            return (
              <div key={i} className="rounded-2xl p-4 animate-slide-up" style={{ background: color.bg, border: `1px solid ${color.border}`, animationDelay: `${i * 60}ms` }}>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2.5 h-2.5 rounded-full" style={{ background: color.dot }} />
                  <span className="text-xs font-bold uppercase tracking-[0.15em]" style={{ color: color.text }}>
                    {d.teams.teamPrefix} {i + 1}
                  </span>
                  <span className="ml-auto text-[10px] text-white/25 tabular-nums">{team.length} {d.teams.people}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {team.map((name, j) => (
                    <span key={j} className="rounded-xl px-3 py-1.5 text-sm font-medium" style={{ background: "rgba(255,255,255,0.09)", color: "rgba(255,255,255,0.8)" }}>
                      {name}
                    </span>
                  ))}
                </div>
          ...
          <button onClick={build} className="w-full py-3 rounded-2xl text-sm text-white/35 hover:text-white/60 border border-white/[0.06] hover:border-white/10 transition-all active:scale-95">
            {d.teams.reshuffle}
          </button>
        </div>
      )}
